...
def add_task(request):
    if request.method=='POST':
        task_obj=Task(task_job=request.POST.get('job'),user_id=request.session['user_id'])
        task_obj.save()
...
def edit_task(request,task_id):
    if request.method=='POST':
        task_obj=Task.objects.filter(id=request.POST.get('id')).update(task_job=request.POST.get('job'))
        return HttpResponseRedirect('/')
    else:
